...
function validateForm(ele='',flag = true)
{
...
function loadPaging1(url='',eShow='',id ='')
{ 
    $('.product-list li a').removeClass('active');
    $('.product-list li a#'+id).addClass('active');
    if(url)
    {
        $.ajax({
            url: url,
...
function load_captcha(){
    ...
    $.ajax({
        ...
        $('.load_captcha').empty().html(data);
        //console.log(data);
    });
}
function check_captcha(cap = null){
    var captcha = $('#contactform-verifycode').val();
    var ses_captcha = cap;
    var flag = true;
    if(captcha != ses_captcha){
        flag = false;
        validateForm("validation-contact",flag);
        $('.help-block-error').addClass('show');
